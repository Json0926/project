...
<!-- 		<div class="form-group">
		    	商品名<input type="text" style="width:120px;" class="form-control" name="Q_EQS_name">
		  	</div> -->
		  	<div class="form-group">
		    	注册时间 <input type="text" style="width:120px;" class="form-control" onclick="WdatePicker()" name="Q_EQD_begintime">
		  	</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			url: 'manageClient/list',
			caption: '<i class="glyphicon glyphicon-list"></i>客户企业信息列表',
			colModel: [
		...
		function addfunc() {
			layer.open({
				...
				area: ['500px', '400px'],
				maxmin: true,
			    content: ['./manageClient/add', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.get('./manageClient/save', form.serialize(), function(json){
			    		//刷新表格
	...
	function editfunc(rowid) {
		...
		layer.open({
			...
			area: ['500px', '350px'],
			maxmin: true,
		    content: './manageClient/get/' + rowid,
		    btn: ['保存', '关闭'],
		    btn1: function(index, layero) {
		    	var form = layer.getChildFrame('form', index);
		    	$.post('./manageClient/save', form.serialize(), function(json){
		    		if(json.success) {
		    			grid.trigger("reloadGrid", [{page: 1}]);
		    			layer.msg('保存成功', {icon: 1});
		    		} else {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./manageClient/del', {ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);

...
<section id="widget-grid" class = "">
	...
	<form id="form-search" class="form-inline" method="post">
		<div class="form-group">
			原材料名<input type="text" style="width:120px;" class="form-control" name="Q_LKS_name">
		</div>
		<div class="form-group">
		  	...
		  	<div class="form-group">
		    	电子邮件 <input type="text" style="width:120px;" class="form-control" onclick="WdatePicker()" name="Q_LED_card.endTime">
		  	</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			colModel: [
				...
				{label: '原材料ID', name: 'materialID', width: 50, editable: true},
				{label: '原材料', name: 'materialName', width: 30},
				{label: '供应商企业ID', name: 'supplierID', width: 30},
				{label: '供应商企业名', name: 'supplierName', width: 50},
				{label: '生产商企业ID', name: 'manufactureID', width: 50},
				{label: '生产商企业名', name: 'manufactureName', width: 50},
				{label: '原材料供应时间', name: 'supplierDate', width: 50},
				{label: '原材料供应数量', name: 'supplierCount', width: 50},
		...
		function addfunc() {
			layer.open({
			    ...
			    btn1: function(index, layero) {
			    	...
			    	$.get('./material/save', form.serialize(), function(json){
			    		//刷新表格
	...
	function editfunc(rowid) {
		...
		layer.open({
			...
			area: ['500px', '350px'],
			maxmin: true,
		    content: './material/get/' + rowid,
		    btn: ['保存', '关闭'],
		    btn1: function(index, layero) {
		    	var form = layer.getChildFrame('form', index);
		    	$.post('./material/save', form.serialize(), function(json){
		    		if(json.success) {
		    			grid.trigger("reloadGrid", [{page: 1}]);
		    			layer.msg('保存成功', {icon: 1});
		    		} else {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./material/del', {ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);

...
<section id="widget-grid" class = "">
	...
	<form id="form-search" class="form-inline" method="post">
		<div class="form-group">
			商品名<input type="text" style="width:120px;" class="form-control" name="Q_LKS_goodsName">
		</div>
		<div class="form-group">
		    	企业名<input type="text" style="width:120px;" class="form-control" name="Q_LKS_manufactureName">
		    	<!-- k:dictSelect name="Q_EQI_card.level.id" className="form-control" itemName="会员卡" headerKey=""-->
		  	</div>
		  	<div class="form-group">
		    	供应时间 <input type="text" style="width:120px;" class="form-control" onclick="WdatePicker()" name="Q_EQD_goodsDate">
		  	</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			colModel: [
				...
				{label: '商品ID', name: 'goodsID', width: 50, editable: true},
		...
		function addfunc() {
			layer.open({
				...
				area: ['500px', '400px'],
				maxmin: true,
			    content: ['pages/gym/cliGoods_form.jsp', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.get('./cliGoods/save', form.serialize(), function(json){
			    		//刷新表格
	...
	function editfunc(rowid) {
		...
		layer.open({
			...
			area: ['500px', '350px'],
			maxmin: true,
		    content: './cliGoods/get/' + rowid,
		    btn: ['保存', '关闭'],
		    btn1: function(index, layero) {
		    	var form = layer.getChildFrame('form', index);
		    	$.post('./cliGoods/save', form.serialize(), function(json){
		    		if(json.success) {
		    			grid.trigger("reloadGrid", [{page: 1}]);
		    			layer.msg('保存成功', {icon: 1});
		    		} else {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./cliGoods/del', {ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);

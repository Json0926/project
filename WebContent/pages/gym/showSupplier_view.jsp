...
/* 		grid.navGrid('#jq_paper',
		...
		top.layer.params = {};
			layer.open({
				...
				area: ['500px', '260px'],
				maxmin: true,
			    content: ['pages/gym/showSupplier_form.jsp', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.post('./showSupplier/save', form.serialize(), function(json){
			    		//刷新表格
			    		if(json.success) {
	...
	top.layer.params = {};
		...
		layer.open({
			...
			area: ['500px', '260px'],
			maxmin: true,
		    content: './showSupplier/get/' + rowid,
		    btn: ['保存', '关闭'],
		    btn1: function(index, layero) {
		    	var form = layer.getChildFrame('form', index);
		    	$.post('./showSupplier/save', form.serialize(), function(json){
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./showSupplier/del', {ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);
	...
	function clearfunc(rowid) {
		layer.confirm('确定置空供应商信息吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./showSupplier/clear/'+rowid, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);
	    			layer.msg('操作成功！', {icon: 1});		
				} else {

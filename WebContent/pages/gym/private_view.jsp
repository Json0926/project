	...
	<div class="row">
		...
		<form id="form-search" class="form-inline" method="post">
		 	...
		 	<!-- <div class="form-group">
		  	...
		  	<div class="form-group">
		    	联系方式 <input type="text" style="width:120px;" class="form-control" name="Q_LKS_name">
		    	<!-- k:dictSelect name="Q_EQI_card.level.id" className="form-control" itemName="会员卡" headerKey=""-->
		  	</div>
		  	<div class="form-group">
		    	电子邮箱<input type="text" style="width:120px;" class="form-control" onclick="WdatePicker()" name="Q_LED_card.endTime">
		  	</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			url: 'private/list',
			caption: '<i class="glyphicon glyphicon-list"></i>重要信息列表',
			colModel: [
				...
				{label: '用户名', name: 'username', width: 50, editable: true},
//				{label: '性别', name: 'sex.itemValue', width:30},
//				{label: '会员卡等级', name: 'card.level.itemValue', width: 30},
				{label: '密码', name: 'password', width: 30},
				{label: '角色', name: 'roleNames', width: 30},
				{label: '企业名', name: 'realname', width: 50},
				{label: '联系电话', name: 'phone', width: 50},
		...
		function addfunc() {
			layer.open({
				...
				area: ['500px', '400px'],
				maxmin: true,
			    content: ['pages/gym/private_form.jsp', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.get('./private/save', form.serialize(), function(json){
			    		//刷新表格
	...
	function editfunc(rowid) {
		...
		layer.open({
			...
			area: ['500px', '350px'],
			maxmin: true,
		    content: './private/get/' + rowid,
		    btn: ['保存', '关闭'],
		    btn1: function(index, layero) {
		    	var form = layer.getChildFrame('form', index);
		    	$.post('./private/save', form.serialize(), function(json){
		    		if(json.success) {
		    			grid.trigger("reloadGrid", [{page: 1}]);
		    			layer.msg('保存成功', {icon: 1});
		    		} else {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./private/del', {ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);
